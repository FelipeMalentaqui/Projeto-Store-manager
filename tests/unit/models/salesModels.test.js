...
const { salesProducts, sales, salesId } = require('./mock/sales.model.mock');
  ...
  describe('testando a findByid', function () {
    ...
    it('Listar apenas com o id inserido', async function () {
     const banco = [{
      ...
      sinon.stub(connection, 'execute').resolves([banco[0]]);
      const id = 1;
      const resolves = await salesModel.findById(1);

      expect(resolves).to.be.deep.equal(salesId[0]);
    });
  });
